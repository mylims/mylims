...
export default class AddonsLoad extends BaseCommand {
  ...
  private async installDependencies (addons: string[]) {
    await Promise.all(addons.map(async addon => {
      ...
      const elnManifestFile = await fs.readFile('package.json', { encoding: 'utf-8' })
      ...
      if(addonManifest.dependencies !== undefined) {
        Object.keys(addonManifest.dependencies)
          .filter(dependencyKey => elnDependenciesKeys.includes(dependencyKey) === false)
          .forEach(dependencyKey => elnManifest.dependencies[dependencyKey] = addonManifest.dependencies[dependencyKey])
      }
      ...
      if(addonManifest.devDependencies !== undefined) {
        Object.keys(addonManifest.devDependencies)
          .filter(devDependencyKey => elnDevDependenciesKeys.includes(devDependencyKey) === false)
          .forEach(devDependencyKey =>
            elnManifest.devDependencies[devDependencyKey] = addonManifest.devDependencies[devDependencyKey]
          )
      }
